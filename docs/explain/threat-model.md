...
- **Rootless Docker.** The daemon runs as the service account, so a privileged container or an escape lands in that account instead of root. It is not a switch: the memory, pids and CPU limits the resource policy sets and the admission counts are only enforced when systemd delegates the cgroup v2 controllers to that user; retained volumes are owned through a subordinate UID range, so an existing installation needs a data migration; and container networking goes through a user mode stack (slirp4netns or pasta), which changes how the loopback ports are published and what they carry. None of this has been run with Sbarbase.
