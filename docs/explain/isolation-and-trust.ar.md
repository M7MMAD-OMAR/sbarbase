...
![زوار التطبيقات غير الموثوقين لا يصلون إلى الخادم إلا عبر وكيل TLS والبوابة بمفتاح عام؛ والمشغّلون الموثوقون يستخدمون لوحة الإدارة أو الخادم مباشرة؛ ولكل بيئة Auth وREST وقاعدة بيانات وحسابات دخول خاصة بها، بينما محرك PostgreSQL وStorage حدود فشل مشتركة](../diagrams/trust-boundaries.ar.svg)

*وكيل TLS وحده يواجه الشبكة، والمحرك وStorage حدود فشل مشتركة.*

![طلب بمفتاح المشروع أ على مسار البيئة أ يصل إلى REST الخاص بها وإلى القاعدة أ؛ والمفتاح نفسه على مسار البيئة ب يتلقى 401 عند البوابة؛ وحساب دخول يحاول قاعدة بيئة أخرى ترفضه قواعد الاتصال في PostgreSQL](../diagrams/key-isolation.ar.svg)

*لا خلط للبيانات: مفتاح البيئة أ ورموزها وحساباتها لا تفتح إلا قاعدتها.*

![دفعة كبيرة على البيئة أ تملأ أماكنها الثمانية المضمونة وتستعير أماكن فارغة، وتبقى آخر 8 من 32 محجوزة، فتُقبل البيئة ب فورًا وتتلقى طلبات أ الزائدة 429؛ وخلف البوابة لكل بيئة حاوياتها بحدود للمعالج والذاكرة والقرص، وحدود اتصالاتها في المحرك المشترك](../diagrams/load-isolation.ar.svg)

*البيئة المزدحمة تستفيد من الخادم الفارغ، لكنها لا تأخذ أبدًا الحصة التي تحتاجها جاراتها. هذه الحدود غير معايرة تحت حمل مستمر.*
