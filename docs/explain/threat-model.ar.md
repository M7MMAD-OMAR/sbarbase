...
- **حساب الخدمة يعادل root.** يضعه [deploy/sbarbase.service](../../deploy/sbarbase.service) في مجموعة `docker` ليتمكن من إدارة الحاويات. هذا قرار مقصود لا سهو، والقسم التالي يعرض الخيارات وأسبابها.
...
يدير المشرف حاوياته عبر واجهة Docker البرمجية. يسحب الصور المثبّتة الإصدار، وينشئ الشبكات ووحدات التخزين، وينشئ الحاويات ويشغّلها، ويشغّل `psql` و`pg_isready` داخل حاوية قاعدة البيانات عبر `docker exec`، ويقرأ `inspect` و`stats` والسجلات. وتقرأ عملية لوحة الإدارة استهلاك الحاويات وسجلاتها عبر المقبس نفسه ([src/control/observe.ts](../../src/control/observe.ts)). تمنح الوحدة هذا الوصول بـ`SupplementaryGroups=docker`. ومن يستطيع الكتابة إلى مقبس Docker يستطيع تشغيل حاوية بـ`--privileged` أو بجذر الخادم `/` مركّبًا فيها، فحساب الخدمة، وأي كود يعمل به، هو root على الخادم فعليًا. لذلك فإن ثغرة تنفيذ كود في المشرف أو العامل أو لوحة الإدارة اختراق للخادم كله، لا للتثبيت وحده.

درسنا ثلاثة خيارات.

- **تشغيل Docker بلا صلاحيات root (rootless).** تعمل خدمة Docker بحساب الخدمة نفسه، فتنتهي الحاوية ذات الصلاحيات أو أي هروب منها عند ذلك الحساب لا عند root. لكنه ليس مفتاحًا يُقلب: حدود الذاكرة والعمليات والمعالج التي تضعها سياسة الموارد ويحسبها القبول لا تُفرض إلا إذا فوّض systemd وحدات تحكم cgroup v2 إلى ذلك المستخدم. وتُملك وحدات التخزين المحفوظة عبر نطاق معرّفات مستخدمين تابع، فيحتاج التثبيت القائم إلى ترحيل بياناته. ويصل المشرف إلى Auth وREST وStorage بعناوينها على شبكة Docker، وهذه الشبكات في وضع rootless تعيش داخل مساحة الشبكة الخاصة بالمستخدم ولا يصل إليها الخادم، فيجب إعادة تصميم طريقة وصول لوحة الإدارة والبوابة إلى الخدمات. لم نجرّب شيئًا من هذا مع صباربيز.
- **وسيط أمام المقبس يرشّح الطلبات.** يستطيع وسيط أمام المقبس أن يسمح ببعض المسارات والأفعال ويرفض الباقي. لكنه لا يجعل الحساب آمنًا هنا. تحتاج طبقة التشغيل إلى إنشاء الحاويات وتشغيلها وexec، والمرشّح الذي يحكم بالمسار والفعل ويسمح بـ`POST /containers/create` يسمح أيضًا بجسم طلب فيه `Privileged`، أو تركيب `/` من الخادم، أو صلاحيات مضافة، أو مساحة PID أو شبكة الخادم، أو جهاز من الخادم. والسماح بـexec يعني تشغيل أي أمر في أي حاوية يعرف المستدعي اسمها، ومنها قاعدة البيانات. تضييق ذلك يحتاج مرشّحًا يقرأ جسم كل طلب ويفحصه (إضافة تفويض في Docker، أو وسيطًا يفهم طلب الإنشاء)، وهذا مكوّن جديد تمر به كل عملية ذات صلاحيات، وأي خطأ فيه يعيدنا إلى root على الخادم.
- **إبقاء مجموعة `docker`.** هذا هو الموقف الحالي.

يبقى حساب الخدمة في مجموعة `docker`، ويعدّه نموذج التهديد هذا root. وما يترتب على المشغّل: شغّل صباربيز على خادم لا يحمل شيئًا يجب حمايته من التثبيت نفسه، وعامل الوصول إلى حساب الخدمة معاملة الوصول إلى root، وأبقِ لوحة الإدارة خلف وسيط TLS على loopback كما يشرح [دليل النشر على الخادم](../guides/server-deployment.ar.md). وDocker بلا صلاحيات root هو الخيار الذي نبدأ بتجربته، لأنه ينقل الحد نفسه بدل أن يضيف مرشّحًا أمامه، ويحتاج إلى تجربة كاملة خاصة به على خادم قبل أن نَعِد بشيء.
