...
![زوار التطبيقات لا يصلون إلا إلى وكيل TLS، ثم إلى لوحة الإدارة أو البوابة؛ كل بيئة حدٌّ قائم بذاته، والمحرك المشترك وStorage حدود فشل مشتركة](../diagrams/trust-boundaries.svg)
...
| B1 | بيئة واحدة تُغرق البوابة | قبول تزامني لكل بيئة بلا طابور، وحد 1 MiB للجسم، ومهلة لقراءة الجسم، ومهلة 15 ثانية للأصل | [tests/concurrency.test.ts](../../tests/concurrency.test.ts)؛ [tests/gateway.test.ts](../../tests/gateway.test.ts) ("oversize stream rejected before upstream"، "slow body deadline ...")؛ [gateway-overload-checks.json](../evidence/gateway-overload-checks.json) |
...
- **لم يُبنَ Studio الخاص بكل بيئة.** له [مواصفة](../engineering/STUDIO-INTEGRATION.md)، لكنه غير مقدَّم. وحين يصل سيحمل سلسلة اتصال بقاعدة البيانات، ويجب ألا يُتاح أبدًا دون مصادقة.
