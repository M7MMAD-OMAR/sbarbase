...
| Full source and target lifecycle rehearsal | **passed on this host** | `docs/evidence/deployment-rehearsal.json`, 11 of 11 checks, 2026-09-20 16:20 to 16:21 (60 s): preflight, built page intact, supervisor start (needed 2 attempts under host pressure), console serving, management identity realm 200, all 8 environment routes 200, 18 live operator bootstrap checks, 14 combined gateway checks, supervised path, clean shutdown, no owned container left running |
...
| HTTPS termination | **passed on this host**, 15 checks | `lab/tls_termination_check.py`: `deploy/console-tls-proxy.ts` terminates TLS (self-signed certificate in the check) in front of a stub serving the real built page; the page and its assets arrive byte for byte over HTTPS, a management route is reachable, HSTS and nosniff are set, the protocol is forwarded as https, plain HTTP gets a 308, and the proxy refuses a world-readable key, a non-loopback upstream and a missing certificate (`docs/evidence/tls-termination.json`). A public certificate and the operator's own proxy are the server's business |
