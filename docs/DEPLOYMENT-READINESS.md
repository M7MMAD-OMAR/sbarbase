...
| Combined runtime admission on this host | passed | `docs/evidence/combined-runtime-admission.json`: 5888 MiB of container limits and 5.75 CPUs admitted at 8943 MiB available, target started in the same run. The rehearsal that followed recorded 8885 MiB available (`docs/evidence/deployment-rehearsal.json`, 17:07) |
...
| Deployment rehearsal in one command | implemented, records refusal honestly | `lab/deployment_rehearsal.py`; the committed run passed 11 of 11 on 2026-09-20 17:07 (`docs/evidence/deployment-rehearsal.json`). A run started while the host is short of memory instead records `Host headroom insufficient` and starts nothing, as measured on this host at 100 MiB and at 8660 MiB against a 8761 MiB plan |
