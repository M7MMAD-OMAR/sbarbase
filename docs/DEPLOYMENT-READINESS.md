...
| Combined rehearsal on this development host | blocked by measured host capacity, not by code | the start needs 8731 MiB (placement 5888 + reserve 2560 + 283 measured for the running source stage) plus its own startup transient. Measured `MemAvailable` over two minutes on this shared host: minimum 7041, median 8700, maximum 8835 MiB, a spread of 1794. Several launches above 8800 were attempted; every one refused at the combined admission with `host_memory_headroom`, each time after cleanly rolling back. The rehearsal now records the refusal with its cause, and retries a pressure-refused start up to `--attempts` |
