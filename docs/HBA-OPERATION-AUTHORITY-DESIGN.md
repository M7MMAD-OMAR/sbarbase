...
Eight unit tests cover file/directory sync ordering and failure, lost register acknowledgment, immutable intent, invalid identities, torn/symlink/public/oversized records and conservative inspection. The image probe now uses a real private journal before registration and observes its exact token after revocation. Twenty image checks and all137 Python tests pass. Host process SIGKILL before/after dispatch, live lease/receipt validation, journal settlement and container replacement remain unimplemented. No journal cleanup or automatic replay API is provided.
