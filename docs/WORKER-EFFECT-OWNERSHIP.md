...
This guarantees ownership only while the direct provisioning process remains alive. Killing that process can still leave a Docker CLI or daemon-side operation running after lock release. The later [durable receipt gate](PROVISIONING-RECEIPTS.md) blocks automatic replay and startup after an uncertain outcome. The later [effect guardian](EFFECT-GUARDIAN.md) adds a deadline and owned-group signals. Docker cancellation and complete active-provisioning crash recovery are not claimed. Next work must contain local descendants and reconcile actual Docker/database state before retrying; killing a Docker client does not prove a daemon-side effect was rolled back.
