...
A [sequential two-database revocation prototype](SQL-PAIR-REVOCATION.md) also passes 29 live checks, including coordinator SIGKILL between commits and retry. It is not an atomic cutoff or integrated recovery. Registry creation and privilege restriction now commit together to remove a default-grant exposure window.

Latest adversarial finding: a real counterexample restores old target authority after a newer claim creates a previously absent database. This is unresolved in the isolated prototype. [The mutation map](PROVISIONING-MUTATION-MAP.md) records this blocker plus HBA and service-driven mutation boundaries. No production recovery was enabled.
