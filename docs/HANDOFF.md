...
A [sequential two-database revocation prototype](SQL-PAIR-REVOCATION.md) also passes 37 live checks, including coordinator SIGKILL between commits and retry. It is not an atomic cutoff or integrated recovery. Registry creation and privilege restriction now commit together to remove a default-grant exposure window.
