...
For the shortest overview, read [START-HERE](START-HERE.md). Unfinished local work: `lab/hba_authority.py` is an isolated, unintegrated prototype with 8 host shell tests and 34 pinned-image checks; no operation-authority guarantee follows from its presence.
