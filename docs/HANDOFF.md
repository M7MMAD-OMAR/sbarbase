...
- Latest recorded suites: 82 Python tests; 67 Bun tests, 377 assertions. The latest nine-check supervisor SIGKILL rehearsal passed for an idle worker and subsequent restart. The two review findings were fixed and reviewed; see the checkpoint. Test counts have different scopes and are not cumulative safety coverage.
...
Direct provisioning effects now retain worker ownership after worker death; see [scope and tests](WORKER-EFFECT-OWNERSHIP.md). A [durable receipt gate](PROVISIONING-RECEIPTS.md) now blocks replay and startup after unknown outcomes. A [parent-bound guardian](EFFECT-GUARDIAN.md) now applies a local deadline and group termination protocol. A [native completion witness](NATIVE-OUTCOME-RECOVERY.md) now recovers a lost acknowledgment without replay. A [read-only inspector](PROVISIONING-INSPECTION.md) now reports evidence for unresolved effects. Stage-specific recovery for unknown outcomes remains open. An uncommitted preflight-only implementation is in progress; read [the exact work-in-progress boundary](PREFLIGHT-RECOVERY-WIP.md) before running or continuing it.
