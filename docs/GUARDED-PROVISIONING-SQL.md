...
The upstream pair probe passes 45 checks with exact disposable cleanup. The full Python suite passes 110 tests. Sanitized evidence is [stored here](evidence/upstream-sql-pair-fence-checks.json). The unchanged recorded Bun checkpoint remains 73 tests/408 assertions.
