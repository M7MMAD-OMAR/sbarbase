...
مسارات قناة التحديث ([الترقيات](../guides/upgrades.ar.md))، في [src/control/http.ts](../../src/control/http.ts) و[src/control/updates.ts](../../src/control/updates.ts). لا يستدعيها إلا مشغّل التثبيت (مالك أو مدير في العميل الذي أُنشئ عند الإعداد)؛ ويتلقى غيره `403`. لوحة الإدارة تقرأ وتطلب فقط: ينفّذ المشرف كل طلب بعد أن يحكم عليه من جديد، وينشر أحكامه في `.lab/upgrades/current.json`.

| الطريقة | المسار | النتيجة |
|---|---|---|
| GET | `/management/v1/updates` | `{data}` وفيها `current` (`version`، `commit`)، و`available` (الإصدار المعروض: `version`، `tag`، `commit`، و`class` بقيمة `safe` أو `attended` أو `rebuild` أو `manual`، و`signed`، و`reasons`، و`notes` بالحقلين `en` و`ar`، و`changes`) أو `null`، و`refusals`، و`skipped` و`newest` (الإصدارات الأحدث التي تخطّاها الفحص)، و`checkedAt`، و`checkError`، و`settings`، و`timezone` (`name`، `offset`)، و`last` (آخر ترقية ومرحلتها)، و`request` (الطلب قيد التنفيذ، وإلا فآخر طلب انتهى)، و`install` (`possible`، `reason`، `acknowledgement`، أو `null` حين لا يُعرض شيء)، و`canRollback` |
| PUT | `/management/v1/updates/settings` | الجسم `{"check": bool, "automatic": bool, "window": {"start": "HH:MM", "end": "HH:MM"}}` بنظام 24 ساعة وبالتوقيت المحلي للخادم. يُرجع `200` مع `data` المحفوظة و`timezone`. ويُرجع `400` مع جملة للإعدادات غير الصالحة، وحين يكون `automatic` مفعّلًا دون `check` |
| POST | `/management/v1/updates/check` | بلا جسم. يُرجع `202 {state: "requested"}`: يشغّل المشرف فحصًا واحدًا |
| POST | `/management/v1/updates/apply` | الجسم `{"version": "X.Y.Z", "acknowledged"?: bool}`. يُرجع `202 {state: "requested"}`. و`400` دون إصدار صالح. و`409` مع جملة حين لم يحكم المشرف على الإصدار بعد، أو لا يُعرض شيء، أو ليس الإصدار هو المعروض، أو يقول الحكم إنه لا يمكن تثبيته الآن (مع سبب الحكم)، أو كان إصدار `attended` و`acknowledged` ليست `true` |
| POST | `/management/v1/updates/rollback` | بلا جسم. يُرجع `202 {state: "requested"}`، أو `409` مع السبب حين لا يمكن التراجع عن آخر ترقية |

كل مسار طلب يُرجع `409` مع "Another update request is still in progress" ما دام طلب آخر يشغل المكان. وبينما ينتظر إصدار جديد فحوص السلامة، تمر القراءة، وتسجيل الدخول إلى الإدارة، و`check`، و`settings`، و`rollback`؛ ويُرجع كل تغيير إداري آخر `409` مع جملة تقول إن التغييرات متوقفة مؤقتًا، وتُرجع مسارات التطبيقات `503` مع `Retry-After: 5`. ولا يمر من الحجز إلا فحص السلامة الخاص بالمشرف، على REST وAuth فقط، برمز خاص بكل تشغيل، ولا يمر أبدًا مع ترويسات `x-forwarded-*` ([src/gateway/hold-bypass.ts](../../src/gateway/hold-bypass.ts)).

## فحص السلامة المحلي

المسار `GET /health` (أو `HEAD`) على المستمع المحلي، للعنوانين `127.0.0.1` و`localhost` فقط، لا يحتاج رمزًا. يُرجع `200 {"status": "ok", "held": bool}` حين تعمل العملية ويُقرأ فهرس التحكم ومخزن المفاتيح كلاهما، و`503 {"status": "unavailable"}` في غير ذلك؛ وأي طريقة أخرى تُرجع `405`. تقول `held` هل حركة التطبيقات محجوزة لأجل تحديث. وتبدأ جولة فحوص السلامة التي يجريها المشرف بعد تحديث بهذا المسار ([src/http/health.ts](../../src/http/health.ts)).
