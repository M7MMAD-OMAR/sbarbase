...
| `.lab/upgrades/state.json` | آخر ترقية: من أين وإلى أين، والمرحلة (`applied`، `confirmed`، `rolling_back`، `rolled_back`، `rollback_failed`، `failed`)، ومن بدأها، ولقطتها، وعدد محاولات الحارس ومرات فشل النقل، وسبب العودة، وفشل التراجع عن إصدار معتمد، وهل هي عالقة، والإشعارات التي يرسلها التشغيل التالي |
...
| `.lab/upgrades/evidence-<time>/` | أدلة كُتبت على هذا الخادم، نُسخت جانبًا قبل أن تنتقل نسخة المستودع (في ترقية أو عودة) |
| `.lab/upgrades/aside-<time>/` | التعديلات المحلية على الملفات المتتبَّعة خارج `docs/evidence/`، والملفات غير المتتبَّعة التي سيكتب الإصدار السابق فوقها، تنسخها العودة إلى هنا قبل أن تنقل نسخة المستودع قسرًا؛ مجلد لكل عودة. لا شيء يحذفها: احذف المجلد بنفسك حين لا تحتاجه |
...
| `.lab/backups/upgrade-moved.json` | تشغيلات النسخ الاحتياطي الخاصة بترقيات نقلت نسخة المستودع. تُحمى آخر 3 منها من الحذف الدوري؛ أما محاولة الترقية التي توقفت قبل الانتقال فنسخها عادية |
...
| `SBARBASE_BACKUP_KEEP` | `7` | عدد النسخ المحفوظة لكل بيئة. لا تُحسب النسخ المأخوذة لترقية: تُحفظ نسخ آخر 3 ترقيات نقلت نسخة المستودع دائمًا |
