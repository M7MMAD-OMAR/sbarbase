...
تقع إعدادات التحديث في لوحة الإدارة وطلباتها وسجل الترقية في `.lab/upgrades/`، خاصة بحساب الخدمة: المجلد 0700، وملفات JSON بصلاحية 0600، ويُستبدل كل منها دفعة واحدة. لا تكتب لوحة الإدارة إلا `settings.json` و`request.json`، ويكتب المشرف الباقي ([الترقيات](../guides/upgrades.ar.md)).
...
| `.lab/upgrades/request.json` | الطلب الوحيد قيد التنفيذ (`apply` أو `rollback` أو `check`)، ولا يُنشأ إلا إذا لم يوجد غيره؛ ويتحقق منه المشرف مرة أخرى قبل التنفيذ |
| `.lab/upgrades/last-request.json` | آخر طلب انتهى، لعرض التقدم في لوحة الإدارة |
| `.lab/upgrades/available.json` | نتيجة آخر فحص: الإصدار الذي يعمل، وأحدث إصدار يمكن الانتقال إليه، وصنفه وملاحظاته وأسباب رفضه |
| `.lab/upgrades/check.json` | وقت آخر فحص، وخطؤه، وعدد مرات الفشل المتتالية (لتباعد المحاولات) |
| `.lab/upgrades/ledger.json` | الإصدارات التي أُعلن عنها، والتي جُرّبت تلقائيًا، والتي تُراجع عنها، فيُعلن عن كل منها مرة واحدة ولا يُعاد تجريبها تلقائيًا |
| `.lab/upgrades/current.json` | ما يعمل الآن، وهل يمكن للوحة الإدارة أن تعرض التراجع، ويكتبه المشرف |
| `.lab/upgrades/state.json` | آخر ترقية: من أين وإلى أين، والمرحلة (`applied`، `confirmed`، `rolling_back`، `rolled_back`، `rollback_failed`، `failed`)، ومن بدأها، ولقطتها |
...
| `.lab/upgrades/upgrade.lock`، `check.log`، `apply.log`، `rollback.log` | القفل الذي تحمله ترقية أو تراجع واحد، ومخرجات آخر عملية فرعية من كل نوع |

يحدد `SBARBASE_RELEASE_SOURCE` مستودع Git الذي يقرأه فحص الإصدارات (عنوان أو مسار)، والافتراضي المستودع الرسمي عبر HTTPS. أيًا كان المصدر، يُتحقق من التواقيع مقابل `deploy/release-signers` في نسخة المستودع التي تعمل. لا يمرره `compose.yaml` إلى الحاوية، فمع Docker أضفه إلى قائمة `environment` فيه.
