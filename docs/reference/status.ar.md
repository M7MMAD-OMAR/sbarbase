...
- الاستعادة إلى نقطة زمنية، وأهداف SSH أو rsync للنسخ خارج الخادم (المدعوم الآن تخزين متوافق مع S3 فقط)، وإعادة بناء خادم مفقود كله من النسخ الخارجية بخطوة واحدة (نسخة كل بيئة تُستعاد، ويسجّل بيان التثبيت ما تحتاجه النسخ، أما حالة التثبيت نفسها فلا تنتقل معها بعد).
...
الخادم الحقيقي: [المرحلة 1 من خارطة الطريق](../engineering/plans/2026-09-23-roadmap.md). على محطة العمل، جرى ترحيل الجيل للقاعدة المحفوظة بإشراف المشغّل في 2026-09-25، وبقيت أعداد الصفوف كلها كما هي ([الدليل](../evidence/generation-migration-retained.json)). يبقى الفحص الحي لإعادة إنشاء الحاويات الدائمة معطّلًا، لأن خطوته التي تحذف كل الحاويات المملوكة ستمنع البيئات المنشورة من الاستئناف، وإعادة تفعيله تنتظر قرار المالك؛ وحتى ذلك الحين يبقى قياسا الضغط المدفوع بالطلبات الواصلة وحمل SDK المختلط متوقفين.
