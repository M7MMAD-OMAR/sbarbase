...
| ترقية إلى PostgREST أحدث عبر `lab/upgrade.py`؛ ثم إصدارات معطوبة يعود منها المشرف وحده (إصدار لا يجيب فيه PostgREST أبدًا، وإصدار يرحّل فهرس التحكم ثم يتوقف، وإصدار لا ينجح في فحوص السلامة أبدًا، وحركة التطبيقات محجوزة بالرمز 503 في أثناء ذلك)؛ ووسوم إصدارات يُقبل منها الموقّع بالمفتاح المسرود ويُرفض غير الموقّع والموقّع بمفتاح غير مسرود. ويبقى المستخدمون والهويات وحاويات Storage والملفات كما هي طوال ذلك ([الترقيات](../guides/upgrades.ar.md)) | [docker-upgrade-checks.json](../evidence/docker-upgrade-checks.json) | 45 |
...
| المشروع الأول: الدخول، والمشروع، وتجهيز بيئة، والمفتاح، وتسجيل مستخدم في Auth عبر supabase-js، وREST وStorage عبر البوابة، وطلب متصفح من نطاق آخر، ورفض المفتاح الملغى بـ401 (أُعيد في 2026-09-25) | [vm-empty-server-first-project.json](../evidence/vm-empty-server-first-project.json) | 15 |
...
### مراحل خارطة الطريق في الآلة الافتراضية، 2026-09-25

الخطوات التي تحتاج خادمًا، جُرّبت في آلة افتراضية من النوع نفسه (4 معالجات افتراضية و6656 MiB، والصور المثبّتة منسوخة من محطة العمل) بالسكربت [lab/vm-milestones.sh](../../lab/vm-milestones.sh) إلى أن يُشترى الخادم. جهة شهادات محلية تقوم مقام الشهادة العامة. الملخص مع النطاق والحدود: [vm-milestones-2026-09-25.json](../evidence/vm-milestones-2026-09-25.json).

| ماذا | الدليل | الفحوص |
|---|---|---|
| منفذ ثابت للوحة الإدارة، ووكيل TLS كوحدة systemd، وإعادة تشغيل، ثم مسار المشروع الأول كاملًا عبر HTTPS | [vm-https-first-project.json](../evidence/vm-https-first-project.json) | 15 |
| الدعوات على Auth الإدارة الحقيقي: الدعوة، والمعاينة، والقبول، والدخول، ورفض صلاحيات المشاهد، والإلغاء، والإزالة | [vm-invitation-check.json](../evidence/vm-invitation-check.json) | 16 |
| نسخ احتياطي للبيئتين وهما تخدمان 13326 طلبًا، لم يفشل منها أي طلب (قواعد بيانات صغيرة، نحو ثانية) | [vm-backup-traffic.json](../evidence/vm-backup-traffic.json) | 11 |
| استعادة النسختين على آلة افتراضية ثانية بالأمرين `sbarbase relink` و`restore`، ودخول المستخدمين القدامى بكلمات مرورهم القديمة | [vm-restore-drill.json](../evidence/vm-restore-drill.json) | 16 |
| ترقية إلى PostgREST أحدث، وإصدار معطوب عاد منه التثبيت وحده، ثم رجوع إلى الإصدارات التي ثُبّت بها | [vm-upgrade-checks.json](../evidence/vm-upgrade-checks.json)، [vm-upgrade-return.json](../evidence/vm-upgrade-return.json) | 10 + 4 |
| حد البيئات: عند 6656 MiB يرفض فحص الذاكرة البيئة الثالثة؛ والاستهلاك الفعلي نحو 258 MiB في الخمول | [vm-environment-limit.json](../evidence/vm-environment-limit.json) | 4 |
| تشغيل متواصل: 60 دقيقة في الخمول مع بيئتين، دون إعادة تشغيل أي حاوية، والذاكرة والقرص والسجلات ثابتة | [vm-soak.json](../evidence/vm-soak.json) | 3 |

كشفت هذه التشغيلات خمسة عيوب، وأُصلحت كلها مع اختبارات: كان فحص الاستلام ينتظر لوحة الإدارة قبل وجود الصور؛ وكان سحب الصورة يستسلم بعد إعادة محاولة واحدة فورية؛ وكان قبول الدعوة يرجع 500؛ ورُفضت أول ترقية بعد فحص الاستلام لأن الفحص يكتب فوق ملفات أدلة متتبَّعة؛ ويمنع SELinux وحدةً تشغّل Bun من `/home`. وفُعّل الإبلاغ الخاص عن الثغرات على GitHub في اليوم نفسه.
...
بُنيت في 2026-09-25 ([الترقيات](../guides/upgrades.ar.md)): وسوم إصدارات موقّعة يُتحقق منها مقابل `deploy/release-signers`؛ وأربعة أصناف تُحسب من الفرق (آمن، و`attended` حين يتغير تثبيت Auth أو Storage أو Realtime، ويحتاج إعادة بناء، ويحتاج ترحيلًا)؛ وإشعار وصفحة تحديثات في لوحة الإدارة لمشغّل التثبيت، مع حكم المشرف على قابلية التثبيت؛ وتثبيت الإصدارات الآمنة بنقرة، وإصدارات `attended` بعد إقرار؛ وتحديث تلقائي اختياري للإصدارات الآمنة داخل نافذة صيانة؛ وتصريف قبل الانتقال؛ وحفظ نسخ آخر 3 ترقيات نقلت نسخة المستودع؛ ولقطة لحالة التحكم؛ وحارس يعمل أولًا في كل تشغيل؛ وحجز لحركة التطبيقات ورفض تغييرات الإدارة بالرمز 409 حتى تنجح خلال 120 ثانية جولة فحوص سلامة تشمل فحوصًا عبر البوابة؛ والعودة.

| ما الذي | الدليل | النتيجة |
|---|---|---|
| قناة الإصدارات وأصنافها، وملفات الطلب والإعدادات والحكم، وقرار التحديث التلقائي ومحاولاته، واللقطة واستعادتها، والحارس، والتصريف، والاعتماد المشروط بالفحوص، ومنطق فحص الترقية في CI نفسه (ملف الترقية يضم أيضًا اختبارات الترقية الأقدم) | `lab/test_release_channel.py`، `lab/test_updates.py`، `lab/test_upgrade.py`، `lab/test_upgrade_health.py`، `lab/test_upgrade_guard.py`، `lab/test_upgrade_drain.py`، `lab/test_upgrade_check.py` | 201 اختبار Python، نجحت على محطة العمل؛ ومجموعة Python كاملة 1004، نجحت |
| حجز الحركة، والفحص الذي يمر من خلاله، ومسارات التحديث، ومنطق صفحة لوحة الإدارة | `tests/hold.test.ts`، `tests/hold-bypass.test.ts`، `tests/updates-routes.test.ts`، `tests/updates-ui.test.ts` | 58 اختبار Bun، نجحت؛ ومجموعة Bun كاملة 293، نجحت |
| حالات CI الثلاث (إصدار يرحّل الفهرس ثم يفشل، وإصدار يفشل في فحوص السلامة، ووسم غير موقّع) | [docker-upgrade-checks.json](../evidence/docker-upgrade-checks.json)، تشغيل CI رقم 36195831433 في 2026-09-25، على جهاز CI نظيف | نجحت 45 فحصًا من 45 |
| تجربة كاملة في الآلة الافتراضية لترقية حقيقية والعودة منها عبر القناة، والحارس، والتصريف، وإصدار `attended` | لا شيء بعد | **لم تُشغَّل** |

عدا اختبارات الوحدات، لم تُشغَّل إلا حالات CI أعلاه، على جهاز CI نظيف وبالأمر `lab/upgrade.py` من سطر الأوامر، لا من لوحة الإدارة ولا بالتحديث التلقائي؛ ولم يُشغَّل أي جزء من القناة في الآلة الافتراضية ولا على خادم حقيقي. ومنذ الصيغة الأولى لهذا القسم صارت كل عودة تنقل نسخة المستودع قسرًا (وتُنسخ التعديلات المحلية إلى `.lab/upgrades/aside-<time>/`)، وصار التراجع الذي يطلبه المشغّل يرفض التعديلات المحلية ويذكر أسماءها، ويتوقف الحارس عن المحاولة بعد 3 مرات نقل فاشلة فإما يشغّل الإصدار السابق، أو يبقى على الإصدار المعتمد، أو يبقى متوقفًا بحالة `stuck`، ولم تعد تحمي نسخها إلا الترقيات التي نقلت نسخة المستودع؛ وكل ذلك مغطى باختبارات الوحدات فقط. لم يُسرد بعد أي مفتاح توقيع في `deploy/release-signers`، فيُرفض كل إصدار على أنه غير موقّع حتى يُسرد مفتاح. وترقية الآلة الافتراضية ([vm-upgrade-checks.json](../evidence/vm-upgrade-checks.json)) سلكت `lab/upgrade.py start --to`، لا القناة ولا لوحة الإدارة ولا التحديث التلقائي.

ولم يعد المشرف الذي يُقتل بدل أن يتوقف (أثناء فحوص السلامة أو في أي وقت آخر) يترك خدمة systemd متوقفة: يوقف التشغيل التالي الحاويات المملوكة التي تركها تعمل، مع إبقاء وحدات التخزين، قبل الفحص المسبق، إلا إذا أمسك قفلها مشرف أو عامل ما زال يعمل، أو انتظر إيصال أو سجل تسويةً (`lab/leftover_runtime.py`، [النشر على خادم](../guides/server-deployment.ar.md)). أُضيف لذلك سطر إلى الوحدة، فلا يسري على وحدة مثبّتة إلا بعد `supervise --apply`. مغطى باختبارات الوحدات فقط، ولم يُجرَّب قتل حي.

وصار كل تشغيل نسخ احتياطي (`create all`، اليومي والذي يسبق كل ترقية) يأخذ أيضًا نسخة من قاعدة Storage المشتركة `storage_metadata` إلى `.lab/backups/storage/`، وتُستعاد بالأمر `lab/backup.py restore-storage`، فيمكن إلغاء إصدار يحتاج تأكيدك ويغيّر تثبيت Storage كليًا باستعادة التشغيل المأخوذ قبله يدويًا ([النسخ الاحتياطي والاستعادة](../guides/backup-and-restore.ar.md)). مغطى باختبارات الوحدات فقط، ولم يُشغَّل على Storage حي.

ويصل الأمر `sbarbase` إلى القناة أيضًا: `upgrade channel`، و`upgrade start --release vX.Y.Z` مع `--allow-class`، و`upgrade rollback --check` ([الأمر sbarbase](../guides/cli.ar.md)).
...
- تجربة على خادم حقيقي: شهادة عامة وDNS، وتشغيل متواصل سبعة أيام بطلبات حقيقية، وقياس السعة تحت الحمل. أما باقي المرحلة الأولى فنجح في آلة افتراضية محلية.
- مجمّع الاتصالات وcron، و`SUPABASE_DB_URL` داخل Edge Functions.
- الاستعادة إلى نقطة زمنية، وأهداف SSH أو rsync للنسخ خارج الخادم (المدعوم الآن تخزين متوافق مع S3 فقط)، وإعادة بناء خادم مفقود كله من النسخ الخارجية بخطوة واحدة (نسخة كل بيئة تُستعاد، وعلى تثبيت جديد بعد أن يعيد `sbarbase relink` إنشاء عميلها ومشروعها وبيئتها بمعرّفاتها الأصلية؛ ويسجّل بيان التثبيت ما تحتاجه النسخ، أما الأعضاء والمفاتيح والإعدادات فلا تنتقل معها بعد، وجُرّب هذا المسار على آلة افتراضية ثانية في 2026-09-25 لبيئات لم تستعمل Studio ولا Realtime ولا الوصول المباشر إلى قاعدة البيانات).
...
- اعتماد أي إصدار من الأصل عبر سياسة التحديث. وتشغيل قناة التحديث على خادم حقيقي أو في الآلة الافتراضية: التحديث التلقائي موجود ومعطّل افتراضيًا، لكن لا تغطيه إلا اختبارات الوحدات وحالات CI (أعلاه).
...
الخادم الحقيقي: [المرحلة 1 من خارطة الطريق](../engineering/plans/2026-09-23-roadmap.md)، لنعيد عليه ما جُرّب في الآلة الافتراضية في 2026-09-25 (أعلاه). على محطة العمل، جرى ترحيل الجيل للقاعدة المحفوظة بإشراف المشغّل في 2026-09-25، وبقيت أعداد الصفوف كلها كما هي ([الدليل](../evidence/generation-migration-retained.json)). نجح فحص دورة الحياة الدائم، بعد إعادة صياغته ليصبح إيقافًا وإعادة بدء للتشغيل المحفوظ دون حذف أي شيء، في 31 تحققًا ([الدليل](../evidence/durable-lifecycle-restart.json)). وعلى البيئتين نفسيهما نجح حمل SDK المختلط دون أي عملية فاشلة ([الدليل](../evidence/sdk-policy-regression.json))، وفشل تشغيل الطلبات الواصلة المستمرة: انتهت 14 من 600 طلب على البيئة المستهدفة دون أي حالة HTTP، بينما جاءت كل طلبات البيئة المجاورة صحيحة ([الدليل](../evidence/gateway-sustained-failure.json)). وُجد السبب في المستمع المحلي وأُصلح دون Docker: كان الاتصال بعد رفض الطلب يُعلَن عنه أنه سيُغلق لكنه يبقى مفتوحًا، فيعيد العميل استخدامه، ثم يقطع تنظيف يجري بعد ثانية واحدة الطلب الطويل التالي عليه ([التفاصيل](../engineering/RESOURCE-POLICY.md)). ثم أُعيد تشغيل الطلبات المستمرة على التشغيل الحقيقي بعد الإصلاح ونجح: 555 رفضًا صحيحًا بالرمز 429، و45 نجاحًا صحيحًا بالرمز 200، وكل طلبات البيئة المجاورة الستين صحيحة، ولم يبقَ أي طلب دون حالة HTTP ([الدليل](../evidence/gateway-sustained-checks.json)). وضع أخذ عينات الضغط ومرحلة الفئة التجريبية لم يُبنيا.
