...
الخادم الحقيقي: [المرحلة 1 من خارطة الطريق](../engineering/plans/2026-09-23-roadmap.md). على محطة العمل، جرى ترحيل الجيل للقاعدة المحفوظة بإشراف المشغّل في 2026-09-25، وبقيت أعداد الصفوف كلها كما هي ([الدليل](../evidence/generation-migration-retained.json)). أُعيدت صياغة فحص دورة الحياة الدائم ليصبح إيقافًا وإعادة بدء للتشغيل المحفوظ دون حذف أي شيء، وأُعيد تفعيله، لكنه لم يُشغَّل بعد؛ ينتظر ذاكرة كافية على الجهاز، ويليه قياسا الضغط المدفوع بالطلبات الواصلة وحمل SDK المختلط.
