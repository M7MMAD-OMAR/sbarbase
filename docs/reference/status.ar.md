...
اختبارات وحدات فقط: لم يُشغَّل أي جزء من القناة تشغيلًا حيًا ولا في الآلة الافتراضية. لم يُسرد بعد أي مفتاح توقيع في `deploy/release-signers`، فيُرفض كل إصدار على أنه غير موقّع حتى يُسرد مفتاح. وترقية CI في جدول Docker أعلاه، وترقية الآلة الافتراضية ([vm-upgrade-checks.json](../evidence/vm-upgrade-checks.json))، سلكتا `lab/upgrade.py start --to`، لا القناة ولا لوحة الإدارة ولا التحديث التلقائي.
