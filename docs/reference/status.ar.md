...
|---|---|---|
| قناة الإصدارات وأصنافها، وملفات الطلب والإعدادات والحكم، وقرار التحديث التلقائي ومحاولاته، واللقطة واستعادتها، والحارس، والتصريف، والاعتماد المشروط بالفحوص، ومنطق فحص الترقية في CI نفسه (ملف الترقية يضم أيضًا اختبارات الترقية الأقدم) | `lab/test_release_channel.py`، `lab/test_updates.py`، `lab/test_upgrade.py`، `lab/test_upgrade_health.py`، `lab/test_upgrade_guard.py`، `lab/test_upgrade_drain.py`، `lab/test_upgrade_check.py` | 214 اختبار Python، نجحت على محطة العمل في 2026-09-26؛ ومجموعة Python كاملة 1041، نجحت |
| حجز الحركة، والفحص الذي يمر من خلاله، ومسارات التحديث، ومنطق صفحة لوحة الإدارة | `tests/hold.test.ts`، `tests/hold-bypass.test.ts`، `tests/updates-routes.test.ts`، `tests/updates-ui.test.ts` | 58 اختبار Bun، نجحت؛ ومجموعة Bun كاملة 295، نجحت |
...
| دورة سطر الأوامر في الآلة الافتراضية التجريبية تحت systemd: `start --to` إلى PostgREST أحدث، وتراجع يطلبه المشغّل، وإصدار معطوب عاد منه صباربيز وحده واستعاد اللقطة | [vm-upgrade-checks.json](../evidence/vm-upgrade-checks.json)، 2026-09-26، الآلة الافتراضية المحلية | نجحت 26 فحصًا من 26 |
| القناة نفسها في الآلة الافتراضية التجريبية كما يستخدمها المشغّل: فحص وتثبيت عبر واجهة الإدارة التي تستخدمها لوحة الإدارة، وإصدار معطوب عاد منه صباربيز وحده، والتحديث التلقائي داخل نافذته، وإصدار `attended` يغيّر Auth بعد الإقرار؛ مع التصريف، والخروج بالرمز 42، وإعادة التشغيل عبر systemd، والحارس، والحجز | [vm-channel-checks.json](../evidence/vm-channel-checks.json)، 2026-09-26، الآلة الافتراضية المحلية، بإصدارات من مستودع محلي موقّعة بمفتاح مؤقت | نجحت 114 فحصًا من 114 |

عدا اختبارات الوحدات، شُغّلت حالات CI أعلاه على جهاز CI نظيف بالأمر `lab/upgrade.py` من سطر الأوامر، وشغّلت الآلة الافتراضية التجريبية المحلية دورة سطر الأوامر والقناة نفسها: إصدارات طُلبت عبر واجهة الإدارة وبالتحديث التلقائي، ونفّذها المشرف تحت systemd. ولم يُشغَّل أي جزء من القناة على خادم حقيقي، ولا مع المستودع الرسمي أو مفتاح توقيع الإصدارات الرسمي، ولم يُشغَّل الزر «Roll back» في لوحة الإدارة، ولا العودة من إصدار `attended`، ولا إصدار يحتاج إعادة بناء، إلا في اختبارات الوحدات. ومنذ الصيغة الأولى لهذا القسم صارت كل عودة تنقل نسخة المستودع قسرًا (وتُنسخ التعديلات المحلية إلى `.lab/upgrades/aside-<time>/`)، وصار التراجع الذي يطلبه المشغّل يرفض التعديلات المحلية ويذكر أسماءها، ويتوقف الحارس عن المحاولة بعد 3 مرات نقل فاشلة فإما يشغّل الإصدار السابق، أو يبقى على الإصدار المعتمد، أو يبقى متوقفًا بحالة `stuck`، ولم تعد تحمي نسخها إلا الترقيات التي نقلت نسخة المستودع؛ وكل ذلك مغطى باختبارات الوحدات فقط. أُدرج مفتاح توقيع الإصدارات الرسمي في `deploy/release-signers` في 2026-09-26؛ والإصدار الأقدم من ذلك الإيداع يرفض كل إصدار على أنه غير موقّع.
...
- اعتماد أي إصدار من الأصل عبر سياسة التحديث. وتشغيل قناة التحديث على خادم حقيقي: التحديث التلقائي موجود ومعطّل افتراضيًا، ولم يُشغَّل إلا في اختبارات الوحدات وحالات CI والآلة الافتراضية التجريبية المحلية (أعلاه).
