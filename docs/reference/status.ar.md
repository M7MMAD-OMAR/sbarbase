...
| حالات CI الثلاث (إصدار يرحّل الفهرس ثم يفشل، وإصدار يفشل في فحوص السلامة، ووسم غير موقّع) | موجودة في مهمة CI، ولا تشغيل مسجّل | **لم تُشغَّل** |
...
اختبارات وحدات فقط: لم يُشغَّل أي جزء من القناة تشغيلًا حيًا ولا في الآلة الافتراضية. ومنذ الصيغة الأولى لهذا القسم صارت كل عودة تنقل نسخة المستودع قسرًا (وتُنسخ التعديلات المحلية إلى `.lab/upgrades/aside-<time>/`)، وصار التراجع الذي يطلبه المشغّل يرفض التعديلات المحلية ويذكر أسماءها، ويتوقف الحارس عن المحاولة بعد 3 مرات نقل فاشلة فإما يشغّل الإصدار السابق، أو يبقى على الإصدار المعتمد، أو يبقى متوقفًا بحالة `stuck`، ولم تعد تحمي نسخها إلا الترقيات التي نقلت نسخة المستودع؛ وكل ذلك مغطى باختبارات الوحدات فقط. لم يُسرد بعد أي مفتاح توقيع في `deploy/release-signers`، فيُرفض كل إصدار على أنه غير موقّع حتى يُسرد مفتاح. وترقية CI في جدول Docker أعلاه، وترقية الآلة الافتراضية ([vm-upgrade-checks.json](../evidence/vm-upgrade-checks.json))، سلكتا `lab/upgrade.py start --to`، لا القناة ولا لوحة الإدارة ولا التحديث التلقائي.
