...
| | Minimum the preflight admits | Comfortable for a few clients |
|---|---|---|
| CPU | 2 cores, x86-64 | 4 cores |
| Memory | 4352 MiB available on an empty server, plus 512 MiB for each environment you add | 8 GB total; 16 GB if you also want room to rehearse a restore on the same machine |
...
One limit does not come from the hardware: a guard kept from the lab allows at most four environments per installation for now (for example two clients with production and staging each). Lifting it waits for measurements on a real server; see the roadmap.
...
- **Oracle Cloud Always Free, Ampere A1.** About 2 OCPU and 12 GB on arm64 ([Oracle's free tier page](https://docs.oracle.com/en-us/iaas/Content/FreeTier/freetier_topic-Always_Free_Resources.htm)). It passes the preflight's figures for an empty server plus a few environments, and every pinned image includes linux/arm64. It has **not** been rehearsed on arm64. Oracle reclaims idle instances and signup needs a payment card for verification; the account is yours to create.
