...
A version from before 2026-09-25 has no release channel, so it cannot see or verify a release. The move onto the first version that has one changes the `Dockerfile`, `compose.yaml` and `deploy/sbarbase.service`, so it is a rebuild and cannot be one click:
