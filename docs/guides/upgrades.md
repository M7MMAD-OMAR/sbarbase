...
Sbarbase tells you in the console when a newer signed release exists. A safe release installs with one click, and a release that changes environment databases installs after you confirm a warning. Every upgrade first lets running work finish and backs up your environments. Application traffic waits until the new version passes its health checks, and Sbarbase returns to the previous version by itself if it does not. You can also let safe releases install themselves inside a maintenance window. That is off by default.
...
An automatic try is used up only once it passes its point of no return, the backup. A try that stopped earlier (a network failure while fetching the release or pulling its images, a refusal, a drain that ran out of time) is tried again, at most 3 times per version, 10 and then 20 minutes apart, inside the window. A try that passed the backup and then failed before the move is not repeated. A version that rolled back, by itself or on request, is never installed automatically, except after a way back caused only by an operation record the previous version had not settled. You can still install any of these from the console.
