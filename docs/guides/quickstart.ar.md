...
يعمل Auth وREST وStorage عبر هذا العنوان الواحد، من خادم أو من صفحة في المتصفح على أي نطاق. ويُشغَّل Realtime لكل بيئة ([Realtime](realtime.ar.md))، ويُضبط مزوّدو OAuth لكل بيئة ([تسجيل الدخول](sign-in.ar.md))، أما Edge Functions فلا تُوجَّه بعد ([مرجع API](../reference/api.ar.md)).
