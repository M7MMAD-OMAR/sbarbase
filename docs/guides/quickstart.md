...
Auth, REST and Storage work through that one address, from a server or from a browser page on any domain. Realtime and Edge Functions are not routed yet; OAuth providers are set per environment ([sign-in](sign-in.md)) ([API reference](../reference/api.md)).
