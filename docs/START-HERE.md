...
The runtime baseline is `155e230`. The isolated `lab/hba_authority.py` prototype now passes 8 host shell tests and 17 pinned-image checks, but remains **unintegrated**. Its next gate is the [HBA authority design](HBA-OPERATION-AUTHORITY-DESIGN.md): immutable operation journal, permanent revocation records, startup/container generation reconciliation and isolated fault tests. Do not enable later-stage automatic replay.
