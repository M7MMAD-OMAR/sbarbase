...
| H3 | **Done 2026-09-25** (`tests/lifecycle.test.ts`, console screens in `ui/Projects.tsx` and `ui/Environments.tsx`): rename (owner or admin; owner for an organization), `POST projects/{id}/move` (owner of both, revokes the project's API keys, keeps `provision_jobs.organization` consistent), and delete (owner) of an empty organization or project and of an environment whose provisioning and supervisor-managed services are idle; a tombstone makes the gateway answer 401 for the deleted runtime, whose data is retained. Not rotated on move: JWT signing key, database password. Reclaiming a deleted runtime is not built. Earlier, 2026-09-23: create organization (installation operators only), list members, retry, with console screens. No routes to create an organization, manage members, move, rename, delete or retry. | `http.ts:123-127` | Add them with the same role checks the catalog already enforces. Delete refuses while an environment is provisioned and revokes its keys. | Deleting an organization that still has projects is refused; a deleted environment's key returns 401. |
...
| H7 | **Done 2026-09-25** (`tests/relink.test.ts`): `ownership` is recorded in the recovery export and in each `lab/backup.py` manifest (since 2026-09-23); `POST /management/v1/relink` and `sbarbase relink <runtime> <backup>` recreate the organization, project and environment on another installation with the same ids and runtime id, matched by id only (a same-name organization with another id, or any other conflict, refuses), then `sbarbase restore` fills the provisioned environment ([backup and restore](../../guides/backup-and-restore.md)). Members, keys and the JWT secret do not travel. The live drill on a second machine is still to run. Recovery exports record only the runtime id. | `lab/recovery-export.py:170` | Add organization, project and environment ids to the bundle manifest. | A restore on a second installation re-links the environment to its project. |
...
The console (`ui/`) covers organization select > projects > environments > connection, with the H3 rename, move and delete screens since 2026-09-25.
