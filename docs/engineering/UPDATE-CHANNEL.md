...
| `lab/upgrade_guard.py` | The start guard: standard library only, copied to `.lab/upgrades/guard.py` by `start`, run first on every start (the unit's first `ExecStartPre`, `deploy/container/start.sh`, else `lab/dev.py run_guard`); counts attempts while a phase is pending and moves back after 3, after an open attempt or on a checkout mismatch |
...
| `ui/Updates.tsx`, `ui/releases.ts` | The notice, the Updates page, the progress view and the settings form. Release notes are shown in Arabic when the browser's `navigator.languages` prefers Arabic, else in English |
...
Workstation, 2026-09-25: `lab/test_release_channel.py`, `lab/test_updates.py`, `lab/test_upgrade.py` (which also holds the older upgrade tests) and `lab/test_upgrade_health.py`, 86 tests, OK; `tests/hold.test.ts`, `tests/updates-routes.test.ts` and `tests/updates-ui.test.ts`, 42 pass; after the console moved to the supervisor's install verdict, 54 pass. After the supervisor's side took on the verdict, the outcome record and the point of no return, the four Python files above with `lab/test_upgrade_guard.py` and `lab/test_upgrade_drain.py` hold 160 tests, OK, and the whole Python suite 978, OK. At 1033cd3, with `lab/test_upgrade_check.py`, the seven files hold 178 tests, OK, and `tests/hold.test.ts`, `tests/hold-bypass.test.ts`, `tests/updates-routes.test.ts` and `tests/updates-ui.test.ts` 58 pass. The three CI cases are now in the CI job (`.github/workflows/ci.yml`), with no recorded run. The existing CI evidence and `lab/vm-milestones.sh upgrade` exercise `start --to` only.
...
- Management changes during the confirmation window are refused with 409 (`src/gateway/hold.ts` `passesHold`), not accepted and then dropped. Schema changes a newer Auth, Storage or Realtime made in environment databases at start are not undone by the way back (the `attended` class), and Storage's shared `storage_metadata` database is in no per-environment backup.
- Under systemd, a supervisor killed while a new version waits for confirmation leaves the owned containers running. The guard moves the checkout back on the next start, but the preflight (`lab/install_server.py check`, `state()`) refuses while owned containers run, so the service can stay down. Read from the code; not rehearsed.
- When the supervisor stops after an update moved the checkout, its final `installation_runtime.py stop` runs from the moved checkout.
...
- The `sbarbase` command has no `channel`, `--release` or `--allow-class`.
