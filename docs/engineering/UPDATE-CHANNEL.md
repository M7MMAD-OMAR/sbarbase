...
Recorded 2026-09-25. Built from the [update channel plan](plans/2026-09-25-update-channel.md). Status: built and covered by unit tests; the three CI cases the plan requires passed on a clean CI machine on 2026-09-25 (CI run 36195831433, 45 of 45 checks, [evidence](../evidence/docker-upgrade-checks.json)). On 2026-09-26 the local rehearsal VM ran the command line cycle under systemd (26 of 26 checks, [evidence](../evidence/vm-upgrade-checks.json)) and then the channel's own operator path: signed releases from a local source, installed from the management API and by automatic mode, a broken one moved back by itself, and an attended Auth release (114 of 114 checks, [evidence](../evidence/vm-channel-checks.json)). A local VM is not a real server, and nothing here is production ready. The operator's view is the [upgrades guide](../guides/upgrades.md).
...
| `deploy/release-signers`, `release.json` | Allowed signers (the maintainer's key since 2026-09-26); the manifest of the running version |
...
Workstation, 2026-09-25: `lab/test_release_channel.py`, `lab/test_updates.py`, `lab/test_upgrade.py` (which also holds the older upgrade tests) and `lab/test_upgrade_health.py`, 86 tests, OK; `tests/hold.test.ts`, `tests/updates-routes.test.ts` and `tests/updates-ui.test.ts`, 42 pass; after the console moved to the supervisor's install verdict, 54 pass. After the supervisor's side took on the verdict, the outcome record and the point of no return, the four Python files above with `lab/test_upgrade_guard.py` and `lab/test_upgrade_drain.py` hold 160 tests, OK, and the whole Python suite 978, OK. After the forced and verified way back, the terminal outcomes of the guard, the supervisor's settling after a failed child and the backup marking, those six files hold 183 tests and `lab/test_backup.py` 20, OK, the whole Python suite 1004, OK, and `bun test` 293 pass. `lab/vm-milestones.sh upgrade` exercises `start --to` from the command line; `lab/vm-milestones.sh channel` exercises the channel as an operator does (see [Rehearsal in the VM](#rehearsal-in-the-vm)). The CI job's run of 2026-09-25 ([docker-upgrade-checks.json](../evidence/docker-upgrade-checks.json)) passed all 45 checks on a clean machine: a confirmed upgrade, three broken versions moved back by themselves (one after migrating the control catalog, whose snapshot restore took the schema from 4 back to 3; one whose PostgREST never answers; one that never passes health, with traffic held with 503 during the window), and unsigned and unlisted-key tags refused with nothing moved. It moved the checkout with `lab/upgrade.py`, not the console or automatic updates.

Acceptance from the plan: the three CI cases (a release that migrates the catalog and then fails, one that starts but fails health, an unsigned tag) passed, above, and the VM rehearsal of a real bump and back through the channel passed on 2026-09-26, below. That is a local VM with test data, not a real server: automatic updates are not described as ready until a real server has run them.

## Rehearsal in the VM

`lab/vm-milestones.sh channel`, in a fresh VM from `lab/vm-rehearsal.sh` (Fedora 44, 6 GiB, the committed HEAD installed under systemd with one environment and a first project). It builds on `lab/upgrade-check.py candidates`, whose `base` lists a throwaway release key in `deploy/release-signers` beside the maintainer's, with the private half under `.git/upgrade-check/`. The installation moves onto `base` with `upgrade.py start --to` and one restart, which also picks up a systemd drop-in setting `SBARBASE_RELEASE_SOURCE` to a bare repository in the guest. Releases are published there one at a time, each a signed tag on top of the one before with a `release.json` for its version:

| Release | What it changes | How it is installed | Outcome |
|---|---|---|---|
| v0.9.1 | PostgREST v14.15 to v14.16 | "check now" and "install" through the management API the console uses, as the operator (the Updates page itself is not driven) | confirmed, trigger `console` |
| v0.9.2 | a PostgREST pin whose image never answers | the same | its runtime does not start; moved back by itself, the control snapshot restored |
| v0.9.3 | nothing but its version | automatic mode, window around the server time, after "check now" | confirmed, trigger `automatic` |
| v0.9.4 | Auth, GoTrue v2.196.0 to v2.197.0 (`attended`) | refused without `acknowledged: true`, then installed with it | confirmed, trigger `console`; the operator signs in again through the migrated management Auth |

Each install is followed from outside the service (`upgrade-check.py channel-apply`, a thread polling the request file, the upgrade state, the console and application traffic): the request runs while the console still answers (the drain), the console goes away, systemd starts Sbarbase again (`NRestarts`, a new invocation, and in the journal `status=42`), the guard counts the new version's start, a fresh control snapshot is taken, and for the three releases that start, application traffic is answered 503 until the confirmation and then served (the broken release never serves: its runtime start fails before the console starts, and its way back follows). Every stage compares users, identities, buckets and files and the control catalog digest with what was recorded before, and reads the `update.*` rows in the audit log and the notification outbox. Automatic mode is shown to leave v0.9.2 (moved back, still on offer) and v0.9.4 (attended) alone for 40 seconds each with its window open.

Result on 2026-09-26: all 114 checks passed in five stages, the move onto the base and then one stage per release, on the committed HEAD in a fresh VM ([vm-channel-checks.json](../evidence/vm-channel-checks.json)). Not covered there: the console's "Roll back", a way back from an attended release (which may need the backups restored by hand), a release that needs a rebuild, the canonical repository and the maintainer's own key, and a real server.

## Known limits

- Unit tests, the CI cases on a clean machine and the local rehearsal VM only, as above; nothing on a real server. The maintainer's release key was listed in `deploy/release-signers` on 2026-09-26; an installation on a version before that commit refuses every release as unsigned and reaches it through `start --to`.
...
- The backup before an upgrade (`backup.py create all --reason upgrade`) holds `storage_metadata` beside the environments since 2026-09-26 (`.lab/backups/storage/<run>/`, its own manifest with the registered tenant ids; restored with `backup.py restore-storage`), so an `attended` release that changes the Storage pin can be undone completely by restoring that run by hand, environments first. Not covered: the way back never restores it by itself; the restore stops Storage for every environment; it is unit tested only; and a previous Storage image that cannot start on migrated metadata still ends in `rollback_failed`, with the database container to be started by hand before the restore can run. `lab/offsite.py` per-environment copies do not carry it (the `backup_offsite.py` run set does), and a version older than this change refuses such a run set as holding an unexpected entry, so after a way back to one, off-host sets written since cannot be fetched; the local pre-upgrade backups are unaffected.
...
- `sbarbase upgrade channel` has no `--preview`. The command gained `channel [--json]`, `start --release vX.Y.Z [--allow-class rebuild|attended]` and `rollback --check` on 2026-09-26; it checks the tag shape and the classes before it passes them to `lab/upgrade.py` as separate arguments.
