...
- The deadline (120 s) starts at the first poll, once the console process exists, so setup before it does not use it up. A round runs every 2 s in a daemon thread, its probes at once (at most 16 together, the detail naming the first failure in probe order); each probe times out after 5 s and goes straight to the upstream service, never through the gateway, with any proxy variable ignored.
