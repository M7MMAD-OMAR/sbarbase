...
The local gateway guarantees 8 requests in flight per environment and admits 32 application requests across one process, without a queue. Since 2026-09-24 the application gateway lets a busy environment borrow up to 24 while 8 of the 32 stay free for environments within their share ([fair share admission](FAIR-SHARE-ADMISSION.md)); the checks below ran against the fixed share of 8, which the probes still construct explicitly. Excess work receives 429 (environment) or 503 (aggregate) with Retry-After. Keys and service routes share the environment budget. Trusted route configuration may additionally cap each service; service, environment and process counts are acquired atomically and released together. The durable lab publishes a REST cap of 3 from the same PGRST_DB_POOL launch configuration. Invalid requests fail before allocation. Management authentication has a separate budget; these limits do not cover every process or socket.
...
[REST cancellation retention](REST-CANCELLATION.md) now prevents ordinary client disconnect from freeing a configured REST slot while its upstream response remains pending. Live SQL observation confirms retention under three cancelled RPCs. SQL execution bounds after upstream failure or deadline remain unproven.
