...
`--cutover` adds the two-stage database fence described in SOURCE-FENCING.md. It journals original scoped login intent, disables service logins before snapshotting and closes all database connections after saving the encrypted artifact. Source service shutdown still affects the complete shared local stack. The integrated option is not yet validated on a full source export and does not itself set catalog maintenance. Default 29-check export evidence predates this option. Preserve journals and explicitly reconcile interrupted operations; never silently reopen source writes.
