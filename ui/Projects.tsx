...
/** Who can act in this organization. Read only: membership changes wait for invitations. */
function Members({organization,request}:{organization:Organization;request:Api}){
 ...
 return <section className="members" aria-labelledby="members-heading"><h2 id="members-heading"><Users aria-hidden="true"/>Members</h2><ErrorMessage message={result.error}/>{result.loading?<Loading/>:result.data&&<div className="table-wrap"><table><thead><tr><th>Operator</th><th>Role</th></tr></thead><tbody>{result.data.data.map(member=><tr key={member.actor}><td><code title={member.actor}>{member.actor.slice(0,8)}</code></td><td>{member.role}</td></tr>)}</tbody></table></div>}</section>;
}
