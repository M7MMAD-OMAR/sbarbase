...
- No legacy host client or queued Docker request may hold HBA intent. Every
  container of that placement must be stopped (checked here, not assumed), the
  relevant locks free and no worker-effect receipt or HBA journal present.
...
lab/verify_retained.py from durable evidence. No secrets or private container
...
def pending(state):
    """Journals and receipts that must be absent before any adoption starts."""
    found=[]
...
def running_placement(owner):
    return docker('ps','-q','--filter','label=io.sbarbase.owner='+owner,check=False).stdout.split()


def script_digest():
    """Identify the code revision that produced a set of checks."""
    return hashlib.sha256((LAB/'adopt-retained.py').read_bytes()+(LAB/'hba_adoption.py').read_bytes()).hexdigest()
...
def main(role):
    ...
    others=running_placement(owner)
    if others:raise SystemExit('Refusing: this placement still has running containers: '+' '.join(others))
    ...
    checks={
        ...
        'no container of this placement is running':not running_placement(owner),
    ...
    # The operation section is replaced wholesale: a check produced by an older
    # code revision never survives into a new claim.
    evidence={**{key:value for key,value in existing.items() if key not in
                ('operation_checks','operation_passed','operation_script_digest','operation_run_at')},
              'role':role,
              'scope':('Retained '+role+' database adoption on the actual stopped container, with only that container started and '
                       'stopped again. Every container of the placement was verified stopped beforehand. Quiescence of legacy host '
                       'clients is an explicit operational assumption. No application service was started. Rule preservation is '
                       'proven byte for byte by lab/verify_retained.py.'),
              ...
              'checkpoints':list(adoption.PHASES),
              'operation_checks':checks,'operation_passed':all(checks.values()),
              'operation_script_digest':script_digest(),
              'operation_run_at':datetime.datetime.now().astimezone().isoformat(timespec='seconds')}
