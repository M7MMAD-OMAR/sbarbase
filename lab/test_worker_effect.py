...
class WorkerEffectTests(unittest.TestCase):
    def test_startup_failures_release_lock_without_effects(self):
        for mismatch in (False,True):
            with self.subTest(mismatch=mismatch),tempfile.TemporaryDirectory() as directory:
                ...
                with path.open('a') as held,(path.parent/'effect.lock').open('a') as lease:
                    ...
                    worker=subprocess.Popen(['bun','-e',script],cwd=ROOT,pass_fds=(held.fileno(),lease.fileno()),env=dict(os.environ,TEST_WORKER_FD=str(held.fileno()),SBARBASE_EFFECT_FD=str(lease.fileno())))
                    held.close();lease.close()
    ...
    def test_failed_or_killed_effect_keeps_pending_receipt_and_refuses_replay(self):
        for code in ("raise SystemExit(1)","import signal;os.kill(os.getpid(),signal.SIGKILL)"):
            with self.subTest(code=code),tempfile.TemporaryDirectory() as directory:
                ...
                with lock.open('a') as held,(lock.parent/'effect.lock').open('a') as lease:
                    fcntl.flock(held,fcntl.LOCK_EX|fcntl.LOCK_NB)
                    ...
                    options=dict(cwd=ROOT,pass_fds=(held.fileno(),lease.fileno()),env=dict(os.environ,TEST_WORKER_FD=str(held.fileno()),SBARBASE_EFFECT_FD=str(lease.fileno())))
    ...
    def test_worker_death_cancels_effect_while_retaining_lock(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with (base/'worker.lock').open('a') as held,(base/'effect.lock').open('a') as lease:
                fcntl.flock(held,fcntl.LOCK_EX|fcntl.LOCK_NB)
                command=['/usr/bin/python3','-c',effect]
                ...
                try:
                    ...
                    held.close();lease.close();worker.kill();worker.wait(timeout=5)
                    ...
                    with self.assertRaises(RuntimeError):effect_lease.acquire(base,timeout=0)
