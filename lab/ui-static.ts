...
export async function uiStatic(request:Request,buildRoot=resolve('.lab/ui')):Promise<Response|undefined> {
 ...
 const isIndex=path==='/',isAsset=/^\/assets\/[A-Za-z0-9_.-]+\.(js|css|svg|webp)$/.test(path);
 ...
 const headers={
  'content-type':isIndex?'text/html; charset=utf-8':path.endsWith('.js')?'text/javascript; charset=utf-8':path.endsWith('.webp')?'image/webp':path.endsWith('.svg')?'image/svg+xml':'text/css; charset=utf-8',
