...
try {
 ...
 else if(['pause','stage','resume'].includes(input.action)) {
  if(input.runtimes.length!==1||!Number.isSafeInteger(input.revision))throw new Error('Invalid pause');
  console.log(JSON.stringify({revision:catalog.changeRuntimeRouting(input.runtimes[0],input.revision,input.action,input.placement)}));
