...
export function managementHandler(catalog:Catalog,identify:ManagementIdentity) {
  return async(request:Request):Promise<Response>=>{
    ...
    const match=path.match(/^\/management\/v1\/(organizations|projects)\/([a-f0-9-]{36})\/(projects|environments)$/);
    if(!match||!((match[1]==='organizations'&&match[3]==='projects')||
      (match[1]==='projects'&&match[3]==='environments'))) return reply(404,{message:'Unknown route'});
    ...
    try {
      ...
      return reply(201,{id:created,state:'metadata_only'});
    } catch(error) {
